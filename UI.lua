...
function draggable(obj) -- https://devforum.roblox.com/t/draggable-property-is-hidden-on-gui-objects/107689/4
    ...
    gui.InputChanged:Connect(function(input)
    	if input.UserInputType == Enum.UserInputType.MouseMovement 或 input.UserInputType == Enum.UserInputType.Touch then
    		dragInput = input
    	end
    end)
    
    UserInputService.InputChanged:Connect(function(input)
    	if input == dragInput 和 dragging then
    		update(input)
    	end
    end) 
end
...
function hovercolor(b,idle,hover,clicked,included)
    ...
    if included 和 typeof(included) == "table" 和 #included > 0 then
...
library.window = function(text)
    ...
    gui.toggle = function(text,default,onclick)
        ...
        b.BackgroundColor3 = Color3.fromRGB(35，35，40)
        b.Text = tostring(text)
        b.TextColor3 = enabled 和 Color3.new(0，1，0) 或 Color3.new(1，0，0)
        b.Size = UDim2.new(1，0，1，0)
        b.FontFace = font
        b.BorderSizePixel = 0
        b.AutoButtonColor = false
    ...
    gui.dropdown = function(text,contents)
        ...
        b.MouseButton1Down:Connect(function()
            ...
            el.ZIndex = d.Visible 和 10 或 12
            b.ZIndex = d.Visible 和 22 或 10
            coldropdown = d.Visible 和 el 或 nil
            
            if not selected then
                b.Text = d.Visible 和 tostring(text).." <" 或 tostring(text).." >"
            else
                b.Text = d.Visible 和 tostring(selected).." <" 或 tostring(selected).." >"
            end
        ...
        subgui.search = function(txt)
            ...
            for i,v in pairs(d:GetChildren()) do
                if v:IsA("TextButton") 和 v.Visible then
                    i = i + 1
                    d.Size = d.Size + UDim2.new(0，0，0，25)
                end
            end
    ...
    gui.slider = function(text,min,max,roundto,default,onchange)
        ...
        button.MouseButton1Down:Connect(function()
            ...
            task.spawn(function()
                while holdingmouse 和 windowalive 和 libalive do
